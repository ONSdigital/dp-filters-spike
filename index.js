  ...
  main.addEventListener('click', function(event) {
    if (event.target && event.target.matches("a")) {
      var url = (event.target).getAttribute('data-url'),
        activeNode = getTaxonomyNode(url),
        nodeChildrenHtml = ["<ul>"];
  ...
  function displayDataForActive() {
    ...
    function buildListOfData(listJSON) {
      ...
      var listHTML = document.createElement('ul'),
        tempHTMLArray = [],
        i,
        listLength = listJSON.result.results.length;
      ...
      $(listJSON.result.results).each(function(index) {
        ...
        resolveDataMetadata(this.uri, success = function(metadata) {
          var thisHTML = $('<a target="_blank" href="https://www.ons.gov.uk' + metadata.uri + '">' + metadata.title + '</a><span>' + metadata.releaseDate +'</span>');
          console.log("Resolved: ", id);
          $('li[data-id="' + id + '"]').html(thisHTML);
        });
        var itemHTML = '<li data-id="' + id + '"></li>';
